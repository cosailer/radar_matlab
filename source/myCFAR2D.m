%% CA-CFAR algorithm 2D
% http://tsaith.github.io/detect-peaks-with-cfar-algorithm.html
% 
% takes a image_w x image_w image, if no detection, data will be 0
% data at the edges of the image is ignored
% 
% guard = 1;
% train_w = 2;
% train_h = 2;
...
function output_image = myCFAR2D(input_image, guard, train_w, train_h, Pfa)
...
for f = 1:image_f
    ...
    for x_main = x_w + 1 : image_w - x_w
        for y_main =  y_h + 1 : image_h - y_h
            ...
            if( input_image(x_main, y_main, f) > cfar_image(x_main, y_main, f) )
                output_image(x_main, y_main, f) = input_image(x_main, y_main, f);
            end
